...
const RenderOrders = ({item})=>{
    return (
        <View style={styles.container}>
            <Image source={JSON.parse(item.item.product_details.product_image)[0]?{uri:`${Constants.BASE_IMAGE_URL}${JSON.parse(item.item.product_details.product_image)[0]}`}:Images.orderImg} style={{flex: 1,
        resizeMode: 'cover'}}/>
